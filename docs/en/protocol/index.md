...
`PacketReader` sits on top of framing, on the read side: given a whole frame's payload, it reads fields in order — VarInts, big-endian fixed-width numbers, length-prefixed UTF-8 strings. This is the abstraction at which per-packet parsers in the Minecraft layer assemble DTOs out of bytes.
