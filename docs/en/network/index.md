...
Four types do the work. `TcpServer` owns the listening socket and the accept loop. `SessionLifetime` drives one connection through the read loop, framing, dispatch, and flush. `IPacketHandler` is the contract that carries a parsed frame up to the application layer. `IPacketHandlerFactory` creates a fresh handler for each connection — so each one owns its per-connection state. Both contracts are defined here, implemented in Minecraft, wired in App.
