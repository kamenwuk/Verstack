...
|---|---|---|---|
| `Handshake` | `0x00` | `HandshakePacketParser.TryParse` → `_phase = Status` or `_phase = Login` (by `nextState`) | `Keep` (or `Disconnect` if the body fails to parse) |
...
| `Login` | `0x00` | `LoginStartPacketParser.TryParse` → stores the name/UUID on the connection | `Keep` (or `Disconnect` if the body fails to parse) |
