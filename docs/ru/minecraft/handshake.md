...
Диспетчер получает разобранный `HandshakePacket` и по `NextState` переключает фазу соединения. Как именно — в [Диспетчере](dispatcher.md). Здесь зафиксирован только факт: `nextState = Status` переводит в фазу Status, `nextState = Login` пока не реализован и логируется, а фаза остаётся Handshake (следующий пакет всё равно уйдёт в ветку по умолчанию и будет проигнорирован).
