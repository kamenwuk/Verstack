...
`TcpServer` владеет слушающим сокетом. `Start()` биндит и слушает; `RunAsync()` крутит accept-цикл. Для каждого принятого сокета он создаёт `SocketConnection` — тот оборачивает сокет в `Pipe` и запускает фоновые циклы приёма — вызывает `_factory.Create()`, чтобы получить свежий handler на это соединение, и передаёт пару (соединение, handler) в `SessionLifetime`.
